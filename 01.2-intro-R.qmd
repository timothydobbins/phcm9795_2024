...
We can use the `hist()` function to produce a frequency histogram, specifying the dataframe to use and the variable to be plotted as `dataframe$variable`:
...
hist(pbc$age, xlab="Age (years)", 
     main="Frequency histogram of participant age from pbc study data")
```

By default, the `hist()` function plots a **frequency histogram**, with counts on the y-axis. We can plot a **density histogram** using the following code:

```{r}
h <- hist(pbc$age, freq=FALSE,
     xlab="Age (years)", 
     main="Density histogram of participant age from pbc study data")
```
