...
huxtable(select(tab_stage_out, 1:4)) |> 
  ...
  theme_article() |> 
  set_width(0.8) |> 
  add_footnote("* Disease stage was missing for 6 participants", border=0)
...
```{r tbl-1-sex-stage-freq, echo=FALSE, warning=FALSE}
...
We can add percentages to two-way tables as either *column* or *row* percents. Using @tbl-1-sex-stage-freq as an example, column percents represent the relative frequencies of sex within each stage: 
...
Conversely, row percents represent the relative frequencies of stage within each sex: 
