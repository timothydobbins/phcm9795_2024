...
b) The mean and its standard error can be obtained using the `descriptives` package, using the `se=TRUE` option:

```{r}
library(jmv)

descriptives(data=bp, vars=diabp, se=TRUE)
```

Note: **we do not use the `descriptives()` function within the `jmv` package to calculate a 95% Confidence Interval for the mean,** as it does not use a t-distribution to calculate the 95% confidence interval. Instead, we use the `t.test()` function (ignoring all output apart from the estimated mean and confidence interval)
