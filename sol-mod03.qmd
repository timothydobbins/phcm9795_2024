...
b) The summary statistics can be obtained using the `t.test()` function 

```{r}
t.test(bp$diabp)
```

Note: **do not use the `descriptives()` function within the `jmv` package,** as it does not use a t-distribution to calculate the 95% confidence interval.
